<template>
    <div
            ...
            'controlsContainer', 'keepCurrentKeyword'
            ]"
            :style="'width:' + this.targetWidth"
    >
        <div v-if="currentKeywordText !== ''"
                class="curKeywordHolder"
        >{{currentKeywordText}}</div>
        <div class="iconHolder" @click="toggleExpanded"><img src="./../../assets/Logo72px.png"/></div>
  ...
  export default {
    ...
    computed:{
      ...
      targetWidth:function(){
        ...
        if(!this.expanded) return '';//If the controls are shown, there's no width
      ...
      currentKeywordText:function(){
        ...
        return currentKeywordKey === '' ? '': this.$store.getters.keyword(currentKeywordKey).strKeywordText;
      }
    },
    methods:{
